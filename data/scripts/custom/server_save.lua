local function serversaves()
	...
	updateGlobalStorage(DailyReward.storages.lastserversaves, os.time())
	-- Reset gamestore exp boost count.
	db.query('UPDATE `player_storage` SET `value` = 0 WHERE `player_storage`.`key` = 51052')
end

local function serversavesWarning(time)
	-- minus one minutes
	...
	if remaningTime > 60000 then
		addEvent(serversavesWarning, 60000, remaningTime)
	else
		addEvent(serversaves, 60000)
	end
end
...
local serversaves = GlobalEvent("serversaves")
function serversaves.onTime(interval)
	local remaningTime = configManager.getNumber(configKeys.SERVER_SAVE_NOTIFY_DURATION) * 60000
	if configManager.getBoolean(configKeys.SERVER_SAVE_NOTIFY_MESSAGE) then
		Game.broadcastMessage("Server is saving game in " .. (remaningTime/60000) .." minute(s). Please logout.", MESSAGE_STATUS_WARNING)
	end
	addEvent(serversavesWarning, 60000, remaningTime)	-- Schedule next event in 1 minute(60000)
	return not configManager.getBoolean(configKeys.SERVER_SAVE_SHUTDOWN)
end
serversaves:time("12:54:00")
serversaves:register()