...
-----------------------aqui para otro scripts--------------------------------
			
		 local specs = Game.getSpectators(Position(33648, 32303, 15), false, false, 10, 10, 11, 11)
    local count = 0
    for i = 1, #specs do
    ...
    if (count >= 1) then
           player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "El Crystal Abrio Pero No Inicio un Nuevo Boss Por Que Hay Moustros Aun Vivos Dentro De la Sala.")
else
startWarzoneIV()
...
-----------------------aqui para otro scripts--------------------------------
		 ...
		 local specs = Game.getSpectators(Position(33680, 32334, 15), false, false, 11, 11, 6, 6)
    local count = 0
    for i = 1, #specs do
        local spectator = specs[i]
        if spectator:isMonster()  then
...
-----------------------aqui para otro scripts--------------------------------
		 ...
		 local specs = Game.getSpectators(Position(33712, 32303, 15), false, false, 10, 10, 10, 10)
    local count = 0
    for i = 1, #specs do
        local spectator = specs[i]
        if spectator:isMonster()  then
