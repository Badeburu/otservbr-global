...
function onUse(player, item)
	...
	if item:getActionId() == 57300 then -- Warzone VI
		...
		for _, spectator in pairs(spectators) do
			if spectator:isPlayer() then
				...
				if jogador:getStorageValue(Storage.DangerousDepths.Acessos.LavaPumpWarzoneVI) < 0 then
		...
		item:transform(transformid[item:getActionId()])
		addEvent(function()
			if item then
	...
	if item:getActionId() == 57302 then -- Warzone V
		...
		for _, spectator in pairs(spectators) do
			if spectator:isPlayer() then
				local jogador = spectator
				if jogador:getStorageValue(Storage.DangerousDepths.Acessos.LavaPumpWarzoneV) < 0 then
	...
	if item:getActionId() == 57301 then -- Warzone IV
		...
		for _, spectator in pairs(spectators) do
			if spectator:isPlayer() then
				local jogador = spectator
				if jogador:getStorageValue(Storage.DangerousDepths.Acessos.LavaPumpWarzoneIV) < 0 then
