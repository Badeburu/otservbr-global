...
local function createVortex()
	...
	if vortexPositions == 0 then
		for i = 1, #positions1 do
			local items = Game.createItem(26127, 1, positions1[i])
			items:decay()
		end
		vortexPositions = 1
	elseif vortexPositions == 1 then
		for i = 1, #positions2 do
			local items = Game.createItem(26127, 1, positions2[i])
			items:decay()
		end
		vortexPositions = 2
	elseif vortexPositions == 2 then
		for i = 1, #positions3 do
			local items = Game.createItem(26127, 1, positions3[i])
			items:decay()
		end
		vortexPositions = 3
	elseif vortexPositions == 3 then
		for i = 1, #positions4 do
			local items = Game.createItem(26127, 1, positions4[i])
			items:decay()
		end
		vortexPositions = 0
	end
	...
	areaCrackler2 = addEvent(createVortex, 12000)
end
-- FUNCTIONS END
