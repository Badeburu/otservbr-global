...
local config = {
    ...
    timeStorage = 23000, -- id of lever before pulled
    pulled_id = 1946 -- id of lever after pulled
}

local player_positions = {
    [1] = {fromPos = Position(33757, 32188, 8), toPos = Position(33750, 32188, 9)},
	[2] = {fromPos = Position(33757, 32189, 8), toPos = Position(33750, 32188, 9)},
	[3] = {fromPos = Position(33757, 32190, 8), toPos = Position(33750, 32188, 9)},
	[4] = {fromPos = Position(33757, 32191, 8), toPos = Position(33750, 32188, 9)},
	[5] = {fromPos = Position(33757, 32192, 8), toPos = Position(33750, 32188, 9)}
}


local configg = {
    maxPlayers = 0,
    room = {fromPos = Position(33715, 32175, 9), toPos = Position(33761, 32201, 9)},
    
}

function boss51Area()
   Game.createMonster("Izcandar the Banished", Position(33726, 32192, 9))
...
if #getPlayersInArea(configg.room.fromPos, configg.room.toPos) > configg.maxPlayers then
  ...
  if player:getStorageValue(config.timeStorage) > os.time() then
    ...
    participants[i].participant:teleportTo(Position(33731, 32192, 9))
    return true
    end
  ...
  addEvent(boss51Area, 500)
    cleanZone(Position(33715, 32175, 9), Position(33761, 32201, 9), true)
    return true
end
